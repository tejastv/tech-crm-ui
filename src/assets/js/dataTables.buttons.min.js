/*!
 ...
 ÂŠ2016-2018 SpryMedia Ltd - datatables.net/license
*/
var $jscomp = $jscomp || {};
$jscomp.scope = {};
...
(function (d) {
  ...
  d.extend(r.prototype, {
    ...
    _buildButton: function (a, b) {
      ...
      a.titleAttr && n.attr("title", k(a.titleAttr));
  ...
  r.background = function (a, b, c, e) {
    ...
    e || (e = m.body);
    a
      ? d("<div/>").addClass(b).css("display", "none").insertAfter(e).fadeIn(c)
      : d("div." + b).fadeOut(c, function () {
      ...
      var c = [],
        ...
        f = function (a, b) {
          ...
          if (d.isArray(a) || a instanceof d)
            for (g = 0, k = a.length; g < k; g++) f(a[g], b);
          else if (null === a || a === l || "*" === a)
            for (g = 0, k = h.length; g < k; g++)
              c.push({ inst: b, node: h[g].node });
          else if ("number" === typeof a)
            c.push({ inst: b, node: b.s.buttons[a].node });
          else if ("string" === typeof a)
            if (-1 !== a.indexOf(","))
              for (h = a.split(","), g = 0, k = h.length; g < k; g++)
                f(d.trim(h[g]), b);
            else if (a.match(/^\d+(\-\d+)*$/))
              (g = d.map(h, function (a) {
                return a.idx;
              })),
                c.push({ inst: b, node: h[d.inArray(a, g)].node });
            else if (-1 !== a.indexOf(":name"))
              for (a = a.replace(":name", ""), g = 0, k = h.length; g < k; g++)
                h[g].name === a && c.push({ inst: b, node: h[g].node });
            else
              d(g)
                .filter(a)
                .each(function () {
  ...
  g.Api.registerPlural("buttons().active()", "button().active()", function (a) {
    return a === l
      ? this.map(function (a) {
  ...
  g.Api.register(["buttons().disable()", "button().disable()"], function () {
  ...
  var w;
  g.Api.register("buttons.info()", function (a, b, c) {
    var e = this;
    if (!1 === a)
      return (
  ...
  var x = function (a) {
    ...
    C = function (a, b) {
      ...
      g = g.nodes().toArray();
      for (
        var m = b.length, n = [], p = 0, q = 0, r = 0 < m ? h.length / m : 0;
        q < r;
        q++
      ) {
        for (var w = [m], u = 0; u < m; u++)
          (w[u] = c.format.body(h[p], q, u, g[p])), p++;
        n[q] = w;
      }
  ...
  d(m).on("init.dt plugin-init.dt", function (a, b) {
    "dt" === a.namespace &&
      (a = b.oInit.buttons || g.defaults.buttons) &&
      !b._buttons &&
      new r(b, a).container();
  });
  g.ext.feature.push({
