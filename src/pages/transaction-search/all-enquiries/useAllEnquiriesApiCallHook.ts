...
//   const queryClient = useQueryClient();
  ...
  const getEnquiries = (isTransactionMaster: boolean): UseQueryResult<AllEnquiriesType[]> => {
    return useQuery<AllEnquiriesType[]>({
      queryKey: [queryKeys.ALL_ENQUIRIES_DATA],
      queryFn: async () => {
        const response = await instance.get(apiUrls.GET_ADD_ALL_ENQUIRY, {
          headers: {
            'X-Transaction-Master': isTransactionMaster ? 'true' : 'false',
          },
        });
