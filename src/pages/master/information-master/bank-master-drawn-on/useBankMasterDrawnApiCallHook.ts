...
export const useBankMasterDrawnApiCallHook = () => {
    ...
    const getBankMasterDrawnOnData = (id: string) => {
        return useQuery<BankdrawnonType>({
    ...
    const addBankMasterDrawnOn = async (
        ...
        const response = await instance.post(apiUrls.GET_ADD_BANKMASTER_DRAWN, BankdrawnOnData);
        return response.data.data;
    };
    ...
    const addBankMasterDrawnOnMutation = () => {
        const mutation = useMutation(
            ...
            {
                onSuccess: () => {
                    queryClient.invalidateQueries({ queryKey: [queryKeys.BANKMASTER_DRAWN_DATA] });
                    navigate("..");
                },
            }
        );
    ...
    const updateBankMasterDrawnOnData = async (
        ...
        const response = await instance.put(
            apiUrls.GET_UPDATE_DELETE_BANKMASTER_DRAWN.replace("{id}", "" + BankMasterDrawnOnData.id),
    ...
    const updateBankMasterDrawnOnMutation = () => {
        const mutation = useMutation(
            (updatedItem: AddBankdrawnonType) => updateBankMasterDrawnOnData(updatedItem),
            {
                onSuccess: () => {
                    queryClient.invalidateQueries({ queryKey: [queryKeys.BANKMASTER_DRAWN_DATA] });
                    navigate("..");
    ...
    const deleteBankMasterDrawnOn = async (id: string): Promise<ApiResponseType<BankdrawnonType>> => {
